...
torch::Tensor online_softmax(torch::Tensor q, torch::Tensor k, torch::Tensor v) {
    ...
    return online_softmax_cuda(q, k, v);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &online_softmax, "Online softmax forward (CUDA)");
}